...
$sql_res=mysqli_query($mysqli, 'SELECT COUNT(*) FROM std_941.horses');
...
$sql='SELECT * FROM std_941.horses LIMIT '.$diapazon.', 10';
if ($_GET['sort'] == 'fam')
$sql='SELECT * FROM std_941.horses ORDER BY name LIMIT '.$diapazon.', 10';
if ($_GET['sort'] == 'birth')
$sql='SELECT * FROM std_941.horses ORDER BY name LIMIT '.$diapazon.', 10';
...
<img src = "data:image/png;base64,' .base64_encode($row['photo']) . '" width = "300px" />
<h3>'.$row['name'].'</h3>
<p>'.$row['breed'].'</p>
<p>'.$row['yearofb'].'гр.</p>
<p>'.$row['about'].'</p></div>';
